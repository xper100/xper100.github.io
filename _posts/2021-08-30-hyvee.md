...
| 정의 | 공식 |
|:---:|:---:|
|재고를 감당하는 주간<br>(Number of Weeks Currently on Hand)| $$ \cfrac{재고(On Hand Balance)}{계절별 주간 재고예측(Seasonalized Weekly Forecast)}$$
...
$Expected Saved Cost = \cfrac{줄인 물량(# of Overstock Cases Prevented) * 줄인 물량의 평균값(Average Cost Per Prevented Cases) * 0.1}{52}$
