...
| 정의 | 공식 |
|:---:|:---:|
|재고를 감당하는 주간<br>(Number of Weeks Currently on Hand)| $$ \cfrac{\displaystyle 재고(On Hand Balance)}{\displaystyle 계절별 주간 재고예측(Seasonalized Weekly Forecast)}$$
...
$$Confidence  = \cfrac{Frequency(X,Y)}{Frequency(X)}$$
...
$$Lift = \cfrac{Support}{Support(X) * Support(Y)}$$ 
